...
class GateL0RD(torch.nn.Module):
    ...
    def __init__(self, input_size, hidden_size, reg_lambda, output_size=-1, num_layers_internal=1,
        ...
        if h_init_net:
            self.f_init = self._create_f_init(f_init_layers=h_init_net_layers, input_dim=input_size,
    ...
    @staticmethod
    def _create_f_init(f_init_layers, input_dim, latent_dim):
    ...
    def _forward_one_step(self, x_t, h_tminus1):
    ...
    def forward(self, x, h_init=None, h_sequence=False):
        ...
        for t in range(S):
            x_t = x[t, :, :]
            y_t, h_t, Theta_t = self._forward_one_step(x_t=x_t, h_tminus1=h_t)
            list_ys.append(y_t)
            list_hs.append(h_t)
